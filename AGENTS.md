...
New this run and worth knowing about: **AWR1843** is the repo's first part with a real vendor errata document (`SWRZ089C`, 48 pp, items headed `ANA#nn`/`MSS#nn` — *not* `Advisory n`, see `KNOWN_SHORTCOMINGS.md`); **LMX2820** is the first part whose pin count and package declaration agree exactly (48/48) and the second independent register-map specimen; **ADC12DJ5200RF** is the largest corpus here (221 pp, 155 sections, 344 tables, 2254 specs, 123 registers, 144 pins — the second part whose pin count and package declaration agree exactly, and the only one so far whose `Pin Functions` table covers two devices at once; see `KNOWN_SHORTCOMINGS.md` and `structure/device_tables._scoped_out`); **SNAA360** is the first document applying to two parts at once (AFE7950 and LMX1204).
...
# onboard (the only commands that may reach the network for a document)
dsa fetch AFE7950                                       # registry -> download -> sha256 verify -> register
dsa fetch --url <URL> --part AD9081                      # the growth path; a miss NEVER guesses a URL
dsa check-revisions --all --json                         # opt-in freshness check; uncached fetcher
dsa diff-rev --part AFE7950 --from "Rev. D" --to "Rev. E" # what moved between two revisions the part holds (no part here holds two)
...
|---|---|---|
| `models.py` | **The contract.** Pydantic v2 models shared by all stages. Change deliberately. | `SourceDocument`, `TOCEntry`, `Footnote`, `TableBlock`, `FigureRef`, `SectionNode`, `RawDocument`, `SectionFile`, `CorpusManifest`, `CorpusStats`, `ExtractionStats`, `GoldenQuestion`, `Project`, `ProjectMember`, `DocType`, `Confidence`, `RECONSTRUCTION_HEADER`/`RECONSTRUCTION_RESCUED`, `SpecUnit`, `SpecRecord`, `SpecTableInfo`, `SpecSet`, `PlotRecord`, `PlotSet`; phase 6 derived artifacts (invariant 8) — `DerivedValue`, `PinRecord`/`PinSet`, `RegisterValue`/`BitRange`/`BitField`/`RegisterRecord`/`RegisterSet`, `CardRow`/`Card`, `ParseConfidence`, `VALUE_KINDS`, `PIN_TYPES`, `CARD_KINDS`, `DERIVATION_VERBATIM`/`DERIVATION_LEXICON`, and the stable-id builders `spec_record_id`/`pin_record_id`/`register_record_id`/`bit_field_id`/`source_ref`; phase 7 errata - `ErrataItem`, `ErrataTarget`, `ErrataTargetKind`, `ErrataLink`, `ErrataLinkSet`, `errata_item_id` |
| `config.py` | pydantic-settings, `DSA_` prefix; `ANTHROPIC_API_KEY` plain. No filesystem side effects at import. `PIPELINE_VERSION`, `SPECS_SCHEMA_VERSION`, `PLOTS_SCHEMA_VERSION`, `SEARCH_SCHEMA_VERSION`, `PINS_SCHEMA_VERSION`, `REGISTERS_SCHEMA_VERSION` and `CARDS_SCHEMA_VERSION` live here, as does `STRUCTURE_STAGE_VERSION` (the version of the structure-stage work that runs *inside* a cached extraction — page and row pinning; it is the extraction cache's second embedded version, beside the backend's `output_version`), as does `card_version` (`DSA_CARD_VERSION`, default `2` — the version of the *derivation rules*, not a schema; it participates in the publish cache key so a changed selector regenerates the card), `ask_budget` (`DSA_ASK_BUDGET`, default 4000 — the `dsa ask` pack budget when `--budget` is not given), `projects_dir` (`DSA_PROJECTS_DIR`, default `projects`), `project_index_token_budget` (`DSA_PROJECT_INDEX_TOKEN_BUDGET`, default 4000 — the hard `PROJECT_INDEX.md` budget) `mcp_max_tokens` (`DSA_MCP_MAX_TOKENS`, default 6000 — the hard cap on every MCP response) and `golden_dir` (`DSA_GOLDEN_DIR`, default empty = this repo's `tests/fixtures` — where the per-part benchmarks live; settable because `dsa golden confirm` writes one, and the test suite points it at a temporary directory so it cannot). `AUDIT_SCHEMA_VERSION` and `GOLDEN_CANDIDATE_SCHEMA_VERSION` also live here. | `Settings`, `get_settings()` (lru_cached; `reset_settings_cache()` for tests) |
...
| `acquire/registry.py` | **The curated document registry as data, not a scraper** (phase 7, ticket 01): `registry/datasheets.yaml`, one entry per part, beside the other checked-in lexicons. No portal scraping, no search-API resolution — a part it does not know raises `RegistryMiss` naming `--url`, and nothing here composes a URL from a part number. Every field is a recorded fact or explicitly absent: `url: null` carries a `url_reason`, a URL nobody fetched is `url_verified: false` with the rule in `url_derivation`, `sha256_origin` distinguishes `local_file:<path>` from `fetch:<url>`, and `retrieved_at` is stamped by a fetch that happened or is null. `DSA_REGISTRY_DIR` points it somewhere else (the *document* registry only; the alias/card/pin-type/device-table lexicons always read the packaged directory). | `DocumentRegistry`, `RegistryEntry`, `RegistryDocument`, `RegistryMiss`, `load_registry`, `save_registry`, `registry_yaml`, `registry_path`, `resolve`, `ti_lit_url` |
| `acquire/fetch.py` | `dsa fetch` — resolve → download → hash-verify → register. **The only command that may reach the network for a document**; `dsa build` acquires nothing, asserted by `tests/unit/test_fetch.py::TestBuildAcquiresNothing`. Downloads through the existing `BinaryFetcher` seam (`ReplayBinaryFetcher` in tests — no second replay mechanism, and nothing here opens a socket). Four refusals: a hash mismatch stops without writing or registering and names its cause from the **parsed revision**, not the hash (`mismatch_cause`); a registry miss names `--url`; bytes are opened as a PDF in memory before anything is staged; a document already in the inventory is skipped and said out loud. | `fetch_part`, `fetch_project`, `fetch_document`, `FetchReport`, `mismatch_cause`, `destination_name` |
| `acquire/revisions.py` | `dsa check-revisions` — the explicit, opt-in, **network** freshness check, and the only writer of a revision state. Revision-first: identifiers differ → `stale`; identifiers equal, bytes differ → `current` + `content_drift` ("regenerated, not revised"); either side unreadable → `unknown` with the reason. A check that cannot complete records only its note and leaves the state and date untouched, so no failure can turn `stale` into `current`. Writes nothing into the part's `documents/` and never rewrites the registry. | `check_part`, `check_all`, `compare`, `RevisionReport`, `refresh_index_banner`, `registry_document_for` |
| `staleness.py` | **One reading, four renderings.** The vocabulary for "is this corpus still the current revision" — `corpus_staleness` resolves a part to its least fresh document (datasheet breaking ties), and `banner_text` is the single sentence `INDEX.md`, `dsa status`, the audit metric and the answer-pack footer all render, so the four surfaces cannot say four different things. `unknown` is the default and is never `current`. The state itself lives on `LibraryDocument.revision_state`, not on `SourceDocument` (frozen by the extraction cache) and not in `sources.json` (derived, regenerated at publish time — a rebuild would clear a `stale` warning). | `CorpusStaleness`, `corpus_staleness`, `load_corpus_staleness`, `banner_text`, `one_line`, `index_banner`, `apply_index_banner`, `status_lines`, `pack_footer`, `audit_metric`, `project_staleness` |
| `vendor.py` | **Vendor routing record, not a rulebook**: profile registry (brand lexicon + backend preference chain), evidence-pinned detection over three sources in strength order — page-1 text, the document's own PDF metadata (where a cover page that prints its brand as a *logo* still carries it: every TI datasheet here is that shape), then the filename — drift warnings. **A vendor is pinned on evidence or not at all**: nothing matched yields `unknown` (routing to the layout floor), never a guess, and detection that establishes nothing never warns drift. No layout behavior hangs off the vendor string. Routing has a second axis that is *not* the vendor: `TABLE_COMPANION_TYPES` (phase 6, ticket 05) names the companion doc types whose tables are the document — register maps — and routes those to `pdf_layout` for every vendor, falling back to `pdf_text` only if the layout backend is unregistered. | `VENDOR_PROFILES`, `TABLE_COMPANION_TYPES`, `detect_vendor`, `select_backend`, `warn_vendor_drift`, `is_known_vendor` |
| `extract/base.py` | Backend protocol + registry. | `ExtractionBackend`, `register`, `get_backend` |
| `extract/pdf_structure.py` | PyMuPDF: content hash, page count, **printed TOC (authoritative page numbers)**, per-page text (verification/pinning only), revision sniffing from a shared lexicon ("Rev."-token shapes — "Rev. 0"/"Rev. A"/"Rev. I", "Rev. N to Rev. M" keeps the last token — plus TI doc-ids). The doc-id shape is deliberately tight — four letters + a 2-3 character tail containing a digit + a final revision letter — because the looser predecessor read the *pin name* `SYSREFOUT0` off lmx1204.pdf p.1 and the heading word `SPECIFICATIONS1` off three Mini-Circuits datasheets; `SUPPORT` fails the digit rule, `SYSREF1` the final-letter rule. `revision_from_texts` is that rule pure, so a document on disk and a download that is never written to disk read one lexicon. Layout analysis lives in `pdf_layout`, never here. | `read_toc`, `page_texts`, `compute_content_hash`, `make_source`, `split_number`, `sniff_revision`, `sniff_revision_bytes`, `revision_from_texts` |
| `extract/http.py` | Fetchers. `CachingFetcher` (disk cache `.cache/http`), `CachingBinaryFetcher` (`.cache/http-bin`), `ReplayFetcher`/`ReplayBinaryFetcher` (hermetic tests: miss = hard error), `MappingFetcher`, and `DirectBinaryFetcher` — **uncached on purpose**, the fetcher `dsa check-revisions` uses, because a freshness question answered out of `.cache/http-bin` reports a previous run's bytes as today's upstream. | `Fetcher` / `BinaryFetcher` protocols |
...
| `extract/pdf_text.py` | Degraded backend for the **prose** companions — errata / app notes: paragraphs only, no trusted tables/figures, contextual page-number stripping. Register maps left this path in phase 6 (ticket 05): their tables *are* the document, so they route to `pdf_layout` like a datasheet. | `PdfTextBackend` |
...
| `structure/quantities.py` | **The parsed numeric layer over the verbatim strings** (phase 6, ticket 02). Strictly additive and always allowed to fail: the printed cell is authoritative and is never mutated. An anchored grammar reads point / range / bound / tolerance shapes, scales SI prefixes, and canonicalises units through `units.py`. `See Figure 7` returns `None`, recorded as `parse_confidence: none` — a first-class outcome, and any consumer that sorts or compares must report the unparsed population rather than drop it. | `parse_quantity`, `annotate_record`, `coverage`, `Quantity`, `ParseCoverage` |
| `structure/device_tables.py` + `registry/device_tables.yaml` | **One abstraction, two consumers** (phase 6, ticket 03): pin tables and register-summary tables are the same structural animal — wide, repetitive, keyed by a first column. Pipeline is identify → map columns → validate → emit, with the header vocabulary in the YAML (data, not code) and positional fallback when headers are abbreviated. A validation failure **rejects the table whole with a recorded reason** into `ExtractionStats.rejection_reasons`, never a half-parsed record set. | `extract_device_tables`, `parse_device_table`, `identify`, `expand_keys`, `parse_address`, `record_rejections`, `KIND_PIN`/`KIND_REGISTER` |
| `structure/plot_axes.py` | **Geometric axis catalog over a document's PDF** (phase 6, ticket 08): tick clusters along a figure region's edges become validated ranges, the rotated span parallel to an edge becomes the axis title. Log axes are marked `low`; anything uncertain stays null. Deterministic and pixel-free — it reads printed text, never the image. | `annotate_plot_axes`, `read_axis_catalog`, `parse_tick`, `axis_contains` |
| `structure/corpus.py` | `RawDocument` → per-section render plans (markdown + CSV twins). | `build_section_plans`, `section_stem`, `slugify` |
| `derive/provenance.py` | **The invariant-8 machinery** (ADR 0007). One `source` format (`<artifact>#<record id>`), one resolver that turns a source string back into the record and printed page it names, one checker that walks any derived artifact and reports every violation. Tolerates a `specs.json` published before ids existed by reconstructing the id from the record's coordinates — ids are pure functions of them. Reads JSON already on disk: no network, no model, no rebuild. The phase's artifact modules (`derive/pins.py`, `derive/registers.py`, `derive/cards.py`, `derive/compare.py`) sit beside it, each exposing one `cli_*(args) -> int` that `cli.py` delegates to by name. | `resolve_source`, `parse_source`, `check_provenance`, `iter_derived_values`, `describe_problems`, `is_derivation_rule`, `SPECS_ARTIFACT`/`PLOTS_ARTIFACT`/`PINS_ARTIFACT`/`REGISTERS_ARTIFACT`/`CARDS_DIRNAME` |
| `derive/pins.py` + `registry/pin_types.yaml` | `RawDocument` → `PinSet` / `pins.json`, and `dsa pins`. First consumer of the device-table abstraction. Multi-pin rows expand (`A1, A2`, `A1–A4`) so every pin is individually citable, with the printed cell kept in `expanded_from`. The type is a label from the checked-in lexicon and names the entry that produced it in `type_evidence`; `unknown` is a legitimate output and a guess is not. The package cross-check compares a printed pin count against the record count and **warns** rather than failing. A part with no readable pin table publishes no file at all. | `build_pins`, `classify_pin_type`, `package_pin_count`, `cross_check_warnings`, `write_pinset`, `load_part_pins`, `find_pins`, `type_counts`, `cli_pins` |
| `derive/registers.py` | `RawDocument` → `RegisterSet` / `registers.json`, and `dsa regs`. Second consumer of the device-table abstraction: the summary shape (address / name / reset / access). Printed cells are copied verbatim; `address.value` / `reset.value` come from `parse_address` (a documented pure function) and stay `None` when the print does not parse. A record's id folds in the document it was printed in (`doc_key`, the content-hash prefix its published directory also carries), because a part may have two documents printing the same map — LMX1204 does — and `(table_index, row_index)` is only unique inside one of them; a record with no `doc_key` keeps the pre-fix id shape so citations already written still resolve. A summary table that fails validation is rejected whole into `ExtractionStats.rejection_reasons`; a document with none publishes no file. What the map never printed is said out loud (missing reset/access column, printed rows that carried no readable address, the unparsed-address population). `--addr` resolves by parsed value, so `0x1A04`, `0x1a04` and `6660` are one question. Bit fields are ticket 06's and arrive through `_bit_fields`, which attaches an accepted field set to the summary record whose name the field table's caption printed; a refused set leaves `fields` empty and is named in the warnings, and two tables naming one register drop both rather than merging. | `build_registers`, `write_registerset`, `load_part_registers`, `find_registers`, `parse_query_address`, `record_register_rejections`, `cli_regs` |
| `derive/bitfields.py` | Phase 6, ticket 06 — parked twice, **un-parked at extractor `tables-10`**. Reads both printed bit-field shapes (the field-description table, and the bit-position diagram where a field's range comes from the header row's *column boundaries*, never from the name cell's text) and accepts a register's field set whole or refuses it whole. Its accuracy gate — the first six registers of `LMX1204_registermap.pdf` in printed order, 100% with no partial credit — is met; recall over that document is 28 of 35 registers at 100% precision. `derive/registers.py` is its only shipping importer: a wrong bit range is acted on by a driver and fails silently, so there is one door in. | `extract_bit_fields`, `parse_bit_range`, `validate_fields`, `read_bit_header`, `iter_bit_field_tables` |
| `derive/cards.py` + `registry/cards.yaml` | **Task-shaped views** (phase 6, ticket 07): `cards/<kind>.json` + `.md` under the *part* (a card composes records from several documents), and `dsa card`. Four kinds — power, thermal, interface, limits — each a selector rule set in the YAML. Every cell is a `DerivedValue`: verbatim copy, documented pure function, or lexicon label, with source + page + derivation. `limits` joins abs-max against recommended by alias-resolved symbol and computes margin **only where both sides parsed**, flagging zero-margin parameters. An empty card is a real answer. `DSA_CARD_VERSION` is in the publish cache key. | `build_card`, `build_part_cards`, `load_or_build_card`, `write_card`, `render_card`, `row_citation`, `audit_card`, `compute_margin`, `cli_card` |
| `derive/compare.py` | **Cross-part comparison** (phase 6, ticket 09), `dsa compare`: rows aligned by alias-resolved symbol, both verbatim values with both page cites, and an SI delta only where both sides parsed. `min` and `max` name which end of a printed range they assert and subtract that end; every other column — `typ`, a lone unnamed `value` — asserts both ends of a range, so `si_delta` returns a number only where the difference is the same at each and otherwise refuses with a stated reason (`range_not_a_scalar`, shared with `dsa diff-rev`). A row one part prints and another does not is reported (`missing_from`), never dropped, and `parse_coverage` names the unparsed population per part. Publishes no file — a comparison is an answer, not an artifact. Its row models (`CompareCell`, `CompareDelta`, `CompareRow`) live in `models.py` because `FamilyIndex` holds them too; the engine and everything else stays here. `compare_specs(..., key_for=SpecKeyFn)` is the **only** additive hook (phase 7, ticket 07): it replaces the join key and the row's `matched_on` sentence and nothing else, so the four rungs, the ambiguity refusal, the delta and the parse population are shared with the family index unchanged. `dsa compare` never passes one. | `compare_parts`, `compare_specs`, `compare_cards`, `render_comparison`, `si_delta`, `range_not_a_scalar`, `audit_comparison`, `Comparison`, `SpecKeyFn`, `cli_compare` |
| `errata/` + `registry/errata.yaml` | **Errata cross-linking** (phase 7, ticket 04). An errata document joins the corpus as a `pdf_text` document; this package is what connects each of its items to the section, spec row, pin or register it invalidates. `lexicon.py` holds the marker words, the section/pin cue words, the thresholds and the per-rule grades as checked-in data; `items.py` segments a document by marker, then by ordinal, then by a per-section floor that cannot fail (an erratum that disappears is the worst outcome available here); `link.py` runs eight rules, every one an **exact** comparison against an identifier the document printed - there is no similarity rung and no threshold to loosen, because an errata item and a datasheet section are written about the same device in the same vocabulary, so resemblance is the null hypothesis rather than evidence; `render.py` writes `ERRATA.md`, the section-file warning banner and the one-line answer-pack warning. Every item is published: `n_items` is the sum of `links` and `unlinked`, and an unplaced item prints under a constant heading. A record target is composed from `TargetDoc.ref_base`, so a document published once into the shared store is cited `@library/docs/<doc>/...` (ADR 0008). `strip_banner` is the inverse of `insert_banner`, used at exactly one surface - the answer pack's supporting excerpt, which must quote what the *datasheet* page prints. | `build_items`, `is_errata`, `build_errata_links`, `links_by_target`, `sections_to_banner`, `render_errata`, `section_banner`, `insert_banner`, `strip_banner`, `pack_warning`, `load_errata_lexicon`, `ERRATA_LINKS_FILENAME`, `ERRATA_MARKDOWN_FILENAME`, `UNLINKED_HEADING` |
| `audit/` + `registry/audit_rubric.yaml` | **The corpus scorecard** (phase 7, ticket 05). `extraction_stats` tells the *builder* how a build went; this package tells the *agent* whether to trust a corpus before answering. Thirteen readings taken off artifacts the corpus already publishes, each graded into A-F against the checked-in rubric and averaged by weight into one letter plus a headline sentence. `rubric.py` loads the YAML and holds no threshold, weight or letter in Python - a metric the file does not carry is not graded, which is what makes "deleting a metric makes it `n/a`" a testable proof rather than a claim. `build.py` holds the readers and the one rule that governs all of them: **a metric that cannot be computed is `n/a` and is excluded from the average** - never zero (which would defame a corpus for a statistic nobody recorded), never full marks (which would flatter one). `render.py` is the per-part scorecard and the fleet table. No model call anywhere in the path, and every metric carries `source` and `derivation` (invariant 8) even though a scorecard is never written to disk. Two departures from the phase-7 lineage, both argued in `build.py`'s docstring: there is no `table_pin_rate` (this branch's manifest records no pinning count, so the metric could only ever say `n/a`), and there **is** an `errata_link_rate` (an errata item that names no record is a known issue an answer will silently omit; zero items is `n/a`, never 100 %). | `build_scorecard`, `METRIC_KEYS`, `Reading`, `AuditRubric`, `MetricRule`, `load_audit_rubric`, `clear_audit_rubric_cache`, `render_scorecard`, `render_fleet`, `grade_order`, `reading`, `RUBRIC_PATH` |
| `families/` + `registry/families.yaml` | **Part families** (phase 7, ticket 07), `dsa family list|suggest|confirm|build` and `--family` as the third scope beside `--part` / `--project`. A series answered once instead of per member: `FAMILY_INDEX.md` lists every section the members print **identically** once, and tabulates only what moved. The rule the whole package turns on is that **membership is DECLARED, never inferred** - `registry.py` refuses `families.candidate.yaml` by *name* before parsing it, forces `confirmed: False` on everything it loads out of one, and refuses an unconfirmed entry even inside `families.yaml`; `suggest.py` may only propose (part-number stem + section-map Jaccard >= 0.8, every entry `confirmed: false`). `build.py` derives the index: sections align on the printed number, then on the printed title (which is what folds a renumbered back matter into one section), and are `shared` only when the title *and* the body are identical in every member, with the `<!-- source: -->` line and the `# N Title` heading stripped first. Spec deltas are **not** re-implemented here - they go through `derive/compare.py`'s `compare_specs` with the additive `key_for=` hook, so a family aligns on the printed row inside its family section (`section  symbol|name|conditions`, character for character) while `dsa compare` keeps aligning unrelated parts on the alias-resolved symbol. One alignment engine, two artifacts. Pins align on the printed designator, registers on the parsed address, bit fields on the printed name inside an aligned register, and **none of the three is ever scored** - a signed difference between two bit patterns means nothing and looks like it means something. `render.py` writes under a hard token budget with nine stages of degradation and a reserved tail (the counts, and any declared member with no corpus) that no stage may drop; `store.py` writes the bounded markdown plus a complete `family.json`. `retrieve/family.py` owns the corpus walk and `FamilyRetriever` (a `ProjectRetriever` by composition); `pack.build_family_pack` adds the one answer rule - a finding every member produced **character for character** comes back once carrying `shared_with`, and anything else comes back per member under `### Per-member differences`. | `build_family_index`, `render_family_index`, `write_family_index`, `section_body`, `section_of`, `suggest_families`, `load_families`, `load_candidates`, `save_families`, `save_candidates`, `resolve`, `FamilyEntry`, `FamilyRegistry`, `FamilyMiss`, `FamilyUnconfirmed`, `FamilyMember`, `MemberRecord`, `CandidatePart`, `STATE_*`, `ALIGNED_*`, `FamilyIndex` / `FamilySection` (in `models.py`), `FamilyRetriever` + `load_members` (in `retrieve/`) |
...
| `publish/writer.py` | Writes corpus + `manifest.json`; also writes `docs/<doc>/specs.json`, `docs/<doc>/plots.json` (when the corresponding sets are supplied) and `docs/<doc>/search_index.json` (always, for every document). Also emits `AGENT.md` beside `INDEX.md` (the ticket-08 protocol; its measured size lands in `CorpusStats.agent_doc_tokens`). Records the index economics in `CorpusStats` (`search_index_bytes` vs `section_bytes`), each section's `search_tokens`, and the part's per-record confidence mix (`spec_confidence` / `plot_confidence`, counts by grade). Also answers the publish-cache-key question for the two artifacts it writes: `specs_current` / `plots_current` (schema-version checks the batch skip gate calls; an absent file reads as current, because a `pdf_text` document publishes neither). Cards are gated differently — `cards_current(part_dir, card_version)` checks `DSA_CARD_VERSION` as well as the schema, because a derivation-rule change alters a card's contents without altering its shape. Errata is gated on the *inventory* rather than on a file: a part that registers an errata document must publish `errata_links.json` (`errata_current`), and a part that registers none must publish no such file at all - an empty one would read as "no known issues", a claim this corpus has no evidence for. The banner an erratum puts on a section is inserted **before** the search index is built, so what is searchable stays exactly what is readable. | `write_corpus`, `doc_dir_name`, `doc_dir_name_for_source`, `specs_current`, `plots_current`, `pins_current`, `registers_current`, `cards_current`, `errata_current`, `write_errata` |
...
| `evalh/golden.py` | Report rendering + token economics measurement, and **where a part's benchmark lives**: `default_golden_path` is the one rule `dsa verify`, `dsa audit` and `dsa golden` all resolve through, so they cannot mean different files by "this part's golden set" (invariant 5). It reads `golden_dir()`, which honours `DSA_GOLDEN_DIR` - `dsa golden confirm` *writes* a benchmark, so the root has to be movable or a test run would edit the objective function it is measured against. `load_golden` refuses a candidate file by name. | `render_verification_report`, `render_spec_query_report`, `render_plot_query_report`, `render_ask_query_report`, `render_search_query_report`, `render_token_economics`, `estimate_lookup_tokens` |
| `evalh/candidates.py` | **The two files `dsa golden suggest\|confirm` own** (phase 7, ticket 06): `golden_qa_<PART>.candidate.yaml` (proposals, every one `confirmed: false`) and `golden_qa_<PART>.rejected.yaml` (the ledger, keyed `<record reference>\|<template>` so a rejection survives regeneration). Neither is the benchmark, and that is defended three ways: the filename, the file's top-level `candidates:` key (it has no `questions:` at all), and `load_golden`'s refusal by name. `question_to_dict` is the single place a golden question becomes YAML, and it writes this branch's `GoldenQuestion` - no `pin_query`, `reg_query` or `card_query`. | `candidate_path`, `rejected_path`, `candidate_key`, `question_to_dict`, `write_candidates`, `read_candidates`, `write_rejections`, `read_rejections`, `rejected_keys`, `existing_question_ids`, `GoldenAssistError`, `CANDIDATE_SUFFIX` |
| `evalh/suggest.py` | **Templating candidates from published records.** Four named rules, one per artifact (`spec_row_value`, `pin_row_name`, `register_row_address`, `plot_caption`); the question text is a format string over printed cells and there is no model call in the path. Selection is a **recursive** round-robin over the stratum tuple (artifact → backend → confidence → section), because a flat pass over the composite stratum lets the stratum *count* decide and a set of twenty then holds one pin. Nothing is dropped in silence: every record no template could use is counted by artifact and by reason into `GoldenCandidateSet.refused` (ADR 0005's unparsed-population clause applied to a selector), and an empty pool names the reason that actually refused the most records. A pin candidate asserts `ask_query: {route: pin}` only when the printed designator is one `retrieve.pack.PIN_DESIGNATOR_RE` could lift out of the question - a datasheet that numbers its pins `1..40` prints no such token, and asserting a path the tool cannot take would make the proposal wrong about the tool rather than about the document. | `suggest_candidates`, `strata_of`, `render_suggestion_report`, `TEMPLATES`, `ARTIFACT_ORDER`, `STRATA_DIMENSIONS`, `DEFAULT_N`, `NO_ID`/`NO_PAGE`/`NO_ANSWER`/`NO_IDENTITY`/`NO_PIXELS`/`BARE_CAPTION` |
| `evalh/confirm.py` | **The human decision, as code.** `apply_decisions` is a pure core (a candidate set and a list of accept/edit/reject decisions in, an outcome out) and `run_interactive` a thin shell over it with injected read/write, so the scripted fleet run and the person at a prompt take one code path. `merge_into_golden` **appends** - the hand-written sets' comments are documentation - and validates the merged text **in memory** before the file is opened for writing, so a merge that would produce unparsable YAML cannot leave the benchmark corrupted (a post-write re-read with rollback is the second line of defence). The created-file header records how the questions were confirmed and nothing stronger: `PROVENANCE_PAGE` only for a `--pdf` walk, `PROVENANCE_CORPUS` for a walk with none, and `PROVENANCE_BULK` - the default - for `--accept-ids` / `--decisions`, where no page was ever shown. `page_context` prefers the printed PDF page and says so in as many words when it can only offer the corpus's own text. | `apply_decisions`, `Decision`, `ConfirmOutcome`, `load_decisions`, `decisions_from_ids`, `merge_into_golden`, `MergeResult`, `page_context`, `PageContext`, `render_candidate`, `run_interactive`, `render_confirm_report`, `PROVENANCE_PAGE`/`PROVENANCE_CORPUS`/`PROVENANCE_BULK`, `EDITABLE` |
| `pipeline.py` | Orchestration + extraction cache (`.cache/extract/<hash>__<backend>.json`, atomic write-temp + rename with a Windows rename retry). A cached record is served only when **both** of its embedded versions still match (`_stale_cache_reason`): the backend's `output_version` and `STRUCTURE_STAGE_VERSION`, because page/row pinning runs after the backend returns and its output is cached too. Vendor routing via the pinned vendor; `--vendor` repins the inventory; drift warnings never re-route. `build_part` takes an additive `on_progress` stage-boundary callback (extracting/structuring/enriching/publishing; no-op default — existing callers unchanged; batch's event emitter wires into it). | `build_part` |
...
| `mcp_server/` | **The second front end** (`dsa serve --mcp`, Phase 5 ticket 07): the corpus as MCP tools over **local stdio** — no HTTP, no auth, no multi-tenancy. `server.py` registers sixteen tools — the nine of phase 5 (`list_parts`, `list_projects`, `get_index`, `search`, `find_spec`, `read_section`, `find_plots`, `get_figure`, `ask`), phase 6's four derived views (`find_pin`, `find_register`, `get_card`, `compare_parts`) and phase 7's three (`list_families` and `get_family_index`, which are `list_projects` and `get_index` one noun across, and `get_audit`, which grades a corpus before an agent answers from it), with `find_plots` gaining the axis filters (`x_label`/`y_label`, `near_x`/`near_y`) that narrow a 514-figure catalog before a vision token is spent — and two index resources (`dsa://part/<PART>/INDEX.md`, `dsa://project/<NAME>/PROJECT_INDEX.md`, registered as templates *and* as concrete resources for what is on disk at start-up). It is bound by the same seam `cli.py` is: every lookup goes through `retrieve/`, every citation comes from `Citation`, every hit shape from that hit's own `as_dict()`, and the pack from `retrieve/pack.py`. `responses.py` is deliberately **SDK-free** — the envelope (`tool`, `scope`, `error` vs `warning`, `truncated`, `over_cap`, `notice`, `citations`, `tokens`), the declared per-tool `SCHEMAS` (checked with `retrieve.pack.validate_pack`, not a second validator) and the response cap live there — the envelope's `scope` object carries all three scope names (`{part, project, family}`) so that every response says which scope answered it, and `search` / `find_spec` / `find_plots` / `ask` each take a `family` beside `part` and `project`, through the same `retrieve.scope.resolve_scope`; `find_pin` and `find_register` do not yet, which since 2026-09-02 makes MCP the *narrower* surface - all seven of the CLI's scoped verbs resolve a family (KNOWN_SHORTCOMINGS.md). Only `ask` folds a common answer; the record lists have nowhere to say whose page a folded row came from. `responses.py` is SDK-free because the `[mcp]` extra is optional and the rule that bounds a response must not vanish with it. `DSA_MCP_MAX_TOKENS` (default 6000) caps every tool response *and* every resource read: list bodies fill greedily in retrieval order, text bodies trim to what is left, and any drop carries a notice naming the setting. Two exceptions are stated rather than hidden: an **image block is atomic** (trimming base64 corrupts a PNG, so the cap governs the JSON that cites it), and an **answer pack shrinks by re-asking at a lower budget**, never by deleting rows from the pack it already built — below a few hundred tokens no pack fits, and the response says `over_cap` instead of shipping a mutilated answer. The `mcp` SDK is imported lazily via the package `__getattr__`, so a core install works and `dsa serve --mcp` without the extra prints an install hint. | `build_server`, `serve_stdio`, `SCHEMAS`, `validate_response`, `response_tokens`, `CAP_SETTING`, `PART_INDEX_URI`, `PROJECT_INDEX_URI` |
| `app/` + `web/` | **The third front end** (`dsa serve`, the local workbench): a FastAPI application plus a React + Vite + TypeScript UI, one process, loopback only (`serve_host` defaults to `127.0.0.1` — there is no authentication, so binding `0.0.0.0` would publish an unauthenticated corpus browser and every NDA document in it). Bound by the **same seam** `cli.py` and `mcp_server/` are, and for the same reason: no retrieval logic, no citation formatting, no JSON shapes of its own. It consumes `retrieve.Retriever` / `ProjectRetriever` **in process** — it does not shell out to the MCP server, whose `DSA_MCP_MAX_TOKENS` cap is correct for a model reading over a wire and wrong for a UI rendering full result sets. `contracts.py` is the frozen HTTP surface: its docstring holds the endpoint table and every request/response model, it imports no FastAPI (so `acquire/applicability.py` stays importable in a plain install), and a router may not invent a shape that is not in it. `main.py` auto-discovers `routers/*.py`; `static.py` serves `web/dist` when it exists and says how to build it when it does not. Two one-way SSE streams (analyze progress, chat tokens) — server-to-client only, so no WebSocket. `app/tools.py` is the chat agent's tool surface: **twelve** tools, pinned by `tests/unit/test_contracts.py` so growing it is a decision rather than an accident. Phase 7's `list_families`, `get_family_index` and `get_audit` are on it, and — unlike their MCP twins — they take **no name**: the scope is resolved once before the loop starts and shown to the user, so `get_family_index` maps the series this turn was scoped to and `get_audit` grades the corpus the answer will come from. `_scope` is shared from `retrieve/scope.py` rather than copied a third time, and `ScopeRef.kind` carries all three scopes (`part | project | family`) so `deps.get_retriever` resolves a series a caller names. **`FamilyRetriever` subclasses `ProjectRetriever`, so every `isinstance` in `tools.py` asks about the family first** — with the checks the other way round a series reported `kind: "project"` on every payload and every part-only refusal called it a design, which is what it did until phase 7's tools reached this surface. `GET /api/families` lists what `registry/families.yaml` *declares* — every entry read through `families.registry.resolve`, so a proposal or an unconfirmed entry never reaches a picker. `scope_resolver.py` matches a declared family only on its exact declared name and only ever as a **candidate**, so no path makes a family the confident scope: a family scope exists because a person chose it. `LibraryDocumentOut` carries `revision_state` whole (not a flattened `staleness` string), so `GET /api/library` reports freshness per document; the Library renders it — `unknown` reads "not checked", as loud as `stale`, never as a quiet tick.. `locate.py` derives highlight geometry **on demand** from the PDF and never persists it (see the `SourceDocument` rule below). The `web` extra is optional exactly as `mcp` is — `dsa serve` without it prints an install hint. | `create_app`, `contracts` (the endpoint table), `JobRegistry`, `resolve_scope`, `locate_citation`, `SessionStore` |
| `protocol.py` | **The corpus agent protocol — one rule text, three destinations** (Phase 5, ticket 08). `RULES`, `CONFIDENCE_HEADER`/`CONFIDENCE_ROWS`, `FALLBACK_RULE`, and (phase 7) `VERBS` + `NETWORK_RULE` are the canonical strings; `build_part_agent_markdown` (written by `publish/writer.py`), `build_project_agent_markdown` (written by `projects/index.py`) and `build_skill_markdown` (the checked-in `.claude/skills/datasheet-corpus/SKILL.md`, regenerated by `scripts/write_skill.py`) render *those exact strings* and add only their own worked examples — the CLI path and the MCP path, scoped to that part or project. `AGENT.md` is bounded like `INDEX.md` is (`AGENT_DOC_TOKEN_BUDGET` = 2200; measured at protocol v2: 1,799 for a part, 1,887 for a three-part project, 2,045 for the worst case the renderers allow, recorded per part in `CorpusStats.agent_doc_tokens`) and carries `PROTOCOL_MARKER`, its embedded version, so `agent_doc_current()` can tell the batch skip gate to republish a corpus written before the protocol existed. | `RULES`, `CONFIDENCE_HEADER`, `CONFIDENCE_ROWS`, `FALLBACK_RULE`, `VERBS`, `NETWORK_RULE`, `AGENT_FILENAME`, `AGENT_DOC_TOKEN_BUDGET`, `PROTOCOL_VERSION`, `PROTOCOL_MARKER`, `SKILL_NAME`, `SKILL_RELPATH`, `rules_block`, `confidence_block`, `verbs_block`, `build_part_agent_markdown`, `build_project_agent_markdown`, `build_skill_markdown`, `write_agent_doc`, `agent_doc_current` |
| `cli.py` | argparse CLI, **formatting only** — it holds no retrieval logic (see the seam under Conventions). `_scope()` turns `--part` / `--project` / `--family` (mutually exclusive, one required) into a `Retriever`, a `ProjectRetriever` or a `FamilyRetriever` and nothing else; project-scoped output labels each hit `[PART]` via the renderers' `show_part`. Reconfigures stdout/stderr to UTF-8 (Windows cp1252). The phase-6 commands (`pins`, `regs`, `card`, `compare`) are **declared here and implemented in `derive/`** — `DERIVED_COMMANDS` maps each to its `cli_*(args) -> int`, and a command whose module has not landed prints why and exits `EXIT_NOT_IMPLEMENTED` (3) instead of tracing back. | `main`, `DERIVED_COMMANDS`, `EXIT_NOT_IMPLEMENTED`, `not_implemented_message` |
...
├── AGENT.md               # the retrieval protocol (~1.8k tok, ceiling 2200; carries its version marker)
...
families/<NAME>/           # DSA_FAMILIES_DIR — gitignored: derived in full from tracked records
├── FAMILY_INDEX.md        # shared sections once, only the deltas tabulated (hard budget, default 4000 tok)
└── family.json            # FamilyIndex: the complete twin, never budget-degraded
...
`families/` is gitignored for a different reason: a family directory holds only
derived files, and the authored half — who is in the series — is the tracked
`registry/families.yaml`. A project directory cannot be ignored the same way,
because `project.json` is authored and nothing else can reconstruct it.
...
6. **Caching keyed by identity.** Extraction cache = (content_hash, backend).
   ...
   record carries one**. Two do: the backend (`extractor_version`, from its
   `output_version`) and the structure stage that runs inside
   `_extract_document` after it (`structure_version`, from
   `config.STRUCTURE_STAGE_VERSION` — table page and per-row page pinning).
   `pipeline._stale_cache_reason` is where both are checked.
7. **Honest degradation.** Missing container → empty section + warning, not a
   ...
   not forced.
8. **Deterministic derived artifacts** (ADR 0007). A derived artifact — pins,
   registers, design cards, comparisons — may contain only **(a)** values
   copied verbatim from a spec, table or pin record; **(b)** values computed
   from those by a documented pure function; **(c)** structural labels drawn
   from a checked-in lexicon (`registry/*.yaml`). Every field carries
   `source` (record id + page) and `derivation` (the named rule that produced
   it) — that is what `DerivedValue` is for. **No model call may appear
   anywhere in the derivation path.** A derived artifact that cannot fill a
   field leaves it **null and says so** (`null_reason`): it never
   interpolates, and it never emits a plausible default. Record ids
   (`specs.json#rec_s4.5-t2-r13`) are pure functions of a record's
   coordinates, so they survive a rebuild of identical input;
   `derive/provenance.py` resolves any `source` back to its record and
   printed page, and `check_provenance()` is the test every derived artifact
   must pass. Any consumer that sorts, compares or checks margins **reports
...
5. A change that alters published output owns the rebuild of the **tracked**
   ...
   `PdfLayoutBackend.output_version` gates the whole extraction cache for the
   documents that backend read; a change under `structure/` that alters what
   `_extract_document` caches is gated by `STRUCTURE_STAGE_VERSION` instead
   (`config.py`), which is what a `ti_html` document's cache entry listens to.
   Bump the one that owns the change — an `output_version` bump says nothing
   about a document the layout backend never read, which is how a per-row
   pinning fix once shipped invisible behind three cached extractions.