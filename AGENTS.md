...
| `cli.py` | argparse CLI, **formatting only** (`dsa pins` and `dsa regs` join `query`/`search`/`ask`/`plots` on the same `--part`/`--project` scope; `dsa card` is deliberately part-only — a design card is one device's and the cross-part view is `dsa compare` — and prints the card's own rendering, exactly as `dsa ask` prints the pack's; `dsa status` prints the manifest's `derived_warnings`, which is where a pin-count mismatch and an empty card are recorded) — it holds no retrieval logic (see the seam under Conventions). `_scope()` turns `--part` / `--project` (mutually exclusive, one required) into a `Retriever` or a `ProjectRetriever` and nothing else; project-scoped output labels each hit `[PART]` via the renderers' `show_part`. Reconfigures stdout/stderr to UTF-8 (Windows cp1252). | `main` |
