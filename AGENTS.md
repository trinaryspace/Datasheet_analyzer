...
- **Publish artifacts are part of the skip gate.** `batch.skip_reason` refuses
  ...
  `PIPELINE_VERSION`. `PINS_` is at **"1"** (phase 6, ticket 04's new `pins.json`), `SPECS_` is at **"4"** (phase 5, ticket 04 added
  ...
  `manifest.json` at publish and is at **"2"** (phase 6, ticket 04 — a wholly
  ...
  forever and never gain the pin table its datasheet prints). A *missing*
  `specs.json`/`plots.json`/`pins.json` reads as current, unlike a
