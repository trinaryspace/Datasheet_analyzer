...
- **Publish artifacts are part of the skip gate.** `batch.skip_reason` refuses
  ...
  `PIPELINE_VERSION`. `PINS_` is at **"1"** (phase 6, ticket 04's new `pins.json`), `REGISTERS_` at **"2"** (ticket 05's new `registers.json`, then ticket 06's bit fields on every one of its records), `SPECS_` is at **"4"** (phase 5, ticket 04 added
  ...
  reads as a document that prints none). A *missing*
