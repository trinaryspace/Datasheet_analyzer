...
- [ ] AD9081 spec tables (partial-ruling Min/Typ/Max, "Test Conditions/Comments" columns) build as atomic tables with correct expanded grids; `dsa query` resolves ADI spec records from specs.json
- [ ] Table pages are pinned and verified against the PDF's own page text
- [ ] A synthetic captionless tabular cluster comes out as paragraphs, never a table
- [ ] A synthetic garbage-spacing layout yields no table at all (gate rejects)
- [ ] `Table N.` captions and test-conditions preambles attach to the correct tables
- [ ] TI-path golden verify remains 100%
