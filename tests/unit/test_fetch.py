...
def settings(tmp_path: Path) -> Settings:
    """A wholly private tree: parts, projects, cache, Library and registry.

    `library_dir` is named explicitly even though `tests/conftest.py`'s autouse
    `isolated_library` already points `DSA_LIBRARY_DIR` here: `dsa fetch`
    registers into the inventory, and an inventory read that fell through to
    the repo's own shelf would make this suite depend on what is checked in.
    """
    return Settings(
...
class TestFetchHappyPath:
    def test_fetch_verifies_registers_and_is_ready_to_build(
        ...
        result = build_part(doc.path, part_number="TEST9000", settings=settings, use_llm=False)
    ...
    def test_a_part_with_companions_fetches_both(self, tmp_path, settings, recorded, reg_file):
        ...
        registry.put(
            RegistryEntry(
                ...
                companions=[
                    RegistryDocument(doc_type=DocType.REGISTER_MAP, url=URL_B, sha256=sha_comp)
                ],
            )
        )
        save_registry(registry, reg_file)
...
class TestHashMismatch:
    ...
    def test_a_matching_hash_is_never_reported_as_accepted(
        ...
        registry = DocumentRegistry()
        registry.put(
            RegistryEntry(part_number="TEST9000", document=RegistryDocument(url=URL_A, sha256=sha))
        )
        save_registry(registry, reg_file)
        report = fetch_part(
            "TEST9000",
...
class TestUrlGrowsTheRegistry:
    ...
    def test_url_with_a_doc_type_lands_as_a_companion(self, tmp_path, settings, recorded, reg_file):
        main = _make_pdf(tmp_path / "main.pdf", "A")
    ...
    def test_a_failed_url_fetch_leaves_no_half_written_entry(self, settings, recorded, reg_file):
        report = fetch_part(
            "NEWPART",
            fetcher=ReplayBinaryFetcher(recorded),
            settings=settings,
...
class TestHermeticFetchSeam:
    ...
    def test_unreadable_bytes_are_a_finding_not_a_corpus(self, settings, recorded, reg_file):
        _record(recorded, URL_A, b"not a pdf at all")
        registry = DocumentRegistry()
        registry.put(RegistryEntry(part_number="TEST9000", document=RegistryDocument(url=URL_A)))
        save_registry(registry, reg_file)
        report = fetch_part(
            "TEST9000",
            fetcher=ReplayBinaryFetcher(recorded),
...
class TestFetchCli:
    ...
    def test_registry_miss_exits_one_and_names_the_flag(self, wired, wired_fetcher, capsys):
