...
def _errata_pdf(tmp_path) -> object:
    """A companion that still routes to the paragraph-only backend.

    It used to be a register map; phase 6 ticket 05 routes those to the layout
    floor, so the honest stand-in for "a document `pdf_text` extracted" is a
    document type that really is still extracted that way.
    """
...
def _build_status_part(tmp_path, datasheet_pdf, settings, with_map=False) -> object:
    ...
    sources = [ds]
    if with_map:
        sources.append(register_source(_errata_pdf(tmp_path),
                                       part_number="P1",
                                       doc_type="errata"))
