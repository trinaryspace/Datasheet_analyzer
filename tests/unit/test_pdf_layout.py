...
def _build(
    ...
    *,
    ...
    return build_part(pdf, part_number=part, settings=settings, vendor=vendor, use_llm=False)
...
class TestStructureLadder:
    def test_numbered_outline_produces_numbered_sections_and_ranges(self, tmp_path):
        pages = [
            ...
            _body(
                ["2 Specifications", "2.1 Electrical Characteristics", "Supply range 3 to 32 V."]
            ),
        ...
        assert _paras(ec) == ["Supply range 3 to 32 V.", "Offset voltage 1 mV typical."]
    ...
    def test_printed_toc_fallback_when_no_bookmarks(self, tmp_path):
        pages = [
            ...
            _body(["Features", "General Description", "Dual op amp with 1234 microvolt offset."]),
            _body(
                [
                    "2 Specifications",
...
class TestFurniture:
    ...
    def test_furniture_free_pdf_keeps_every_line(self, tmp_path):
        pages = [
            _body(
                [
                    "First line with 1500 in the middle.",
                    "Second line is a lone number 42.",
                    "Rev. 0 pattern shaped prose to be preserved.",
                ]
            ),
            _body(["Only content on page two.", "Another 42 style number line."]),
        ]
    ...
    def test_pattern_shaped_body_line_survives(self, tmp_path):
        pages = [
            ...
            _top(["DATA SHEET"])
            + _body(["Worth of a second page alone."])
            + _bottom(["Rev. A | 2 of 3"]),
            _top(["DATA SHEET"])
            + _body(["A third page for the staircase."])
...
class TestMechanics:
    def test_rebuild_uses_pdf_layout_extraction_cache(self, tmp_path):
        pages = [
            _top(["DATA SHEET"]) + _body([f"Body of page {i}."]) + _bottom([f"Rev. 0 | {i} of 4"])
            for i in range(1, 5)
        ]
    ...
    def test_manifest_records_pdf_layout_backend_and_pinned_vendor(self, tmp_path):
        pages = [_body(["One solitary content line."]), _body(["Another page of content."])]
