...
- `tests/fixtures/golden_qa_<PART>.candidate.yaml` — generated proposals, every
  one of them written `confirmed: false`;
- `tests/fixtures/golden_qa_<PART>.rejected.yaml` — the ledger of candidates a
  human turned down, so the same bad candidate is not re-suggested next run.

Neither is the benchmark. `tests/fixtures/golden_qa_<PART>.yaml` is, and the
three names are deliberately siblings: `dsa verify` resolves the benchmark by
...
def candidate_key(source: str, template: str) -> str:
    ...
    rebuild of identical input (`provenance`), so this key is stable — which is
    the whole reason a rejection can be remembered at all.
    """
...
def read_candidates(path: Path) -> GoldenCandidateSet:
    ...
    if not path.exists():
        raise GoldenAssistError(
            f"no candidate file at {path} — generate one first: "
            "`dsa golden suggest --part <PART>`"
        )
    data = yaml.safe_load(path.read_text(encoding="utf-8"))
    if not isinstance(data, dict) or "candidates" not in data:
        raise GoldenAssistError(
            f"{path} is not a candidate file (no top-level `candidates:` key)"
        )
...
def read_rejections(path: Path) -> GoldenRejectionLedger:
    ...
    file, which means the caller asked to confirm something that was never
    generated.
    """
    path = Path(path)
    if not path.exists():
...
def write_rejections(
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        header + dump_yaml(ledger.model_dump(mode="json")), encoding="utf-8"
    )
    return ledger
