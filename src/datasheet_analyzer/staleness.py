...
documents, and one rendering of it per surface, so the four places a designer
...
- **A hash difference is never staleness.** A vendor that regenerates a
  datasheet's package-materials addendum with the current date on every
  download changes the bytes of an *unchanged* revision daily. Staleness is
...
**Where the state lives, and why it is not on `SourceDocument`.** The lineage
this was ported from put six fields on `SourceDocument` and wrote them into
`sources.json`. Neither works here. `SourceDocument` is embedded in every
cached `RawDocument` under `.cache/extract/` and its field set is pinned by
`tests/unit/test_contracts.py::test_source_document_shape_is_unchanged`; and
`sources.json` is a *derived* artifact on this branch (ADR 0005), regenerated
at publish time from the Library, so a check that wrote there would be erased
by the next build and would not even be read back - `load_inventory` answers
from the Library. So the reading lives on `LibraryDocument.revision_state`,
keyed by `content_hash` in `<library_dir>/<content_hash>.json`, beside
`applicability` and `labels`, which are there for exactly the same reason: they
are the mutable, user-or-check-owned metadata of a *document*, not of a build.
One consequence worth naming: a document two parts share carries one reading,
because upstream has one answer about it.
"""
...
from datasheet_analyzer.models import DocType, LibraryDocument, Staleness
...
class CorpusStaleness:
    ...
    #: could not run, or the drift note). Carried verbatim from the record.
    note: str = ""
...
def document_staleness(doc: LibraryDocument, part: str = "") -> CorpusStaleness:
    """One document's reading, straight off its Library record."""
    state = doc.revision_state
    return CorpusStaleness(
        part=part or doc.source.part_number,
        state=state.staleness,
        built_revision=doc.source.revision,
        upstream_revision=state.upstream_revision,
        checked_at=state.checked_at,
        content_drift=state.content_drift,
        note=state.note,
        doc_type=doc.source.doc_type.value,
    )


def corpus_staleness(documents: Sequence[LibraryDocument], part: str = "") -> CorpusStaleness:
    ...
    def rank(doc: LibraryDocument) -> tuple[int, int]:
        # Same state: the datasheet speaks for the part.
        return (
            _SEVERITY[doc.revision_state.staleness],
            0 if doc.source.doc_type == DocType.DATASHEET else 1,
        )

    return document_staleness(min(documents, key=rank), part)


def load_corpus_staleness(part_dir: Path | str, *, store=None) -> CorpusStaleness:
    """`corpus_staleness` over a part directory's Library view.

    Read-only: a legacy `sources.json` entry the Library does not hold yet is
    read but not written back, because reporting a state is not the act that
    should migrate a shelf.
    """
    from datasheet_analyzer.acquire.inventory import library_view

    part_dir = Path(part_dir)
    return corpus_staleness(library_view(part_dir, store=store), part_dir.name)
...
def project_staleness(readings: Sequence[CorpusStaleness]) -> CorpusStaleness:
