...
from datasheet_analyzer.derive.compare import FLAG_AMBIGUOUS, FLAG_ONLY_IN
from datasheet_analyzer.families.build import STATE_PARTIAL, section_of
from datasheet_analyzer.models import CompareCell, CompareRow, FamilyIndex
...
def _render(
    ...
    *,
    ...
    if artifacts:
        ...
        out += _delta_table(REGISTER_HEADING, index.register_deltas, index.members)
...
def _counts(index: FamilyIndex) -> list[str]:
    ...
    if index.unparsed:
        lines.append(
            f"- **{len(index.unparsed)} alignments and value pairs were refused** "
            f"rather than guessed at; they are listed under "
            f"*{NOT_COMPARABLE_HEADING.strip('# ')}* with their printed values."
        )
    lines.append("")
    return lines
...
def _divergent(index: FamilyIndex, *, listing: bool) -> list[str]:
    ...
    for section in divergent:
        ...
        for part in section.members:
            ...
            lines.append(f"  - {part}:{shown} `{section.files[part]}` ({section.pages[part]})")
    lines.append("")
    return lines
...
def _delta_table(
    ...
    *,
    ...
    `derive/compare.py` prints one: a value quoted without the page it came from
    ...
    for row in shown:
        ...
        note = "; ".join(row.not_comparable)
        if note:
            lines.append(f"| | | {_escape(note)} " + "| " * len(members) + "|")
...
def _escape(text: str) -> str:
    """A pipe inside a printed condition must not open a table column."""
    return text.replace("|", r"\|").replace("\n", " ")


def _row_label(row: CompareRow) -> str:
    """The printed identity of a row, plus the section it was printed in.

    Never the join key: a family's spec key is `section + separator + normalized
    identity`, which is exactly the right thing to align on and exactly the
    wrong thing to print at a reader.
    """
    label = row.symbol or row.label or "(unnamed row)"
    section = section_of(row)
    if section:
        label = f"{label} <br>_§{section}_"
    flags = [f for f in (FLAG_ONLY_IN, FLAG_AMBIGUOUS) if f in row.flags]
    return f"**{_escape(label)}**" + ("".join(f" `{f}`" for f in flags))


def _first_role(row: CompareRow) -> str:
    """The printed column this row is read on: the one a delta was computed on,
    else the first column any member filled."""
    if row.deltas:
        return row.deltas[0].cell
...
def _citation(cell: CompareCell) -> str:
    """`§4.9, p.24` for one member's side of a row, or `p.?` when unpinned."""
    pages = f"p.{cell.page}" if cell.page else "p.?"
    section = cell.values[next(iter(cell.values))].section if cell.values else ""
    return f"§{section}, {pages}" if section else pages


def _cell_text(cell: CompareCell, role: str) -> str:
    ...
    body = _escape(value.verbatim) or EM_DASH
    return f"{body} <br>`{_citation(cell)}`"


def _delta_text(row: CompareRow) -> str:
    ...
    for delta in row.deltas:
        if delta.value.value_si is None:
            continue
        sign = "+" if delta.value.value_si > 0 else ""
        parts.append(
            f"{delta.part_number} {sign}{_num(delta.value.value_si)} "
            f"{delta.value.unit_si} *(derived)*"
        )
...
def _refusals(index: FamilyIndex, *, listing: bool) -> list[str]:
    ...
    lines = [
        ...
        "",
        (
            f"{len(index.unparsed)} alignments and value pairs this tool refused "
            f"rather than guessed at, with what each member actually printed."
        ),
        "",
    ]
    if not listing:
...
def _conventions(index: FamilyIndex) -> list[str]:
    return [
        ...
        (f'- Ask the whole family at once: `dsa ask --family {index.name} "<question>"`.'),
        "",
    ]
