...
def section_signature(sections) -> frozenset[tuple[str, str]]:
    ...
    return frozenset((section.number.strip(), normalize(section.title)) for section in sections)
...
def suggest_families(
    ...
    *,
    ...
    for group in groups:
        ...
        if not name:
            continue
        worst = min(overlap(a, b) for i, a in enumerate(group) for b in group[i + 1 :])
        out.append(
            FamilyEntry(
                name=name,
...
def _pairs(a: CandidatePart, b: CandidatePart, *, min_overlap: float, min_stem: int) -> bool:
