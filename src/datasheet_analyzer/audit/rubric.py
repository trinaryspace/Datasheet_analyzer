...
class AuditRubric:
    ...
    def overall(
        ...
        score = sum(
            (self.grade_points(grade) or 0.0) * weight for grade, weight in graded
        ) / total_weight
...
def _rule(key: str, body: object) -> MetricRule | None:
    ...
    try:
        ...
        log.warning(
            "audit rubric: metric %r declares kind %r, which is not a metric kind "
            "— not graded", key, body.get("kind"),
        )
        return None
