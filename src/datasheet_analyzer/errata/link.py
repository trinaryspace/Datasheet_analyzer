...
def build_errata_links(
    ...
    *,
    ...
    if not targets:
        notes.append(
            "this part published no document for an erratum to point at, so no item "
            "could be linked"
        )
    empty_reason = ""
    if not items:
...
def _link_item(
    ...
    return ErrataLink(
        errata_item_id=item.id, item=item, targets=out, notes=notes
    )
...
def _record_target(
    ...
    *,
    ...
    """
    ref = (
        source_ref(record.id, artifact=artifact, doc=doc.name) if record.id else ""
    )
    return ErrataTarget(
        kind=kind,
        id=ref,
