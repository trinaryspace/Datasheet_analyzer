...
class RegistryDocument(BaseModel):
    ...
    def _null_reads_as_empty(cls, data):
        if isinstance(data, dict):
            data = {k: ("" if v is None and k in _NULL_AS_EMPTY else v) for k, v in data.items()}
        return data
...
def _document_yaml(doc: RegistryDocument) -> dict:
    ...
    out["retrieved_at"] = doc.retrieved_at.isoformat() if doc.retrieved_at is not None else None
    if doc.filename:
...
def registry_yaml(registry: DocumentRegistry) -> str:
    ...
    return FILE_HEADER + yaml.safe_dump(payload, sort_keys=False, allow_unicode=True, width=100)
