...
`datasheet_analyzer.compare` is imported **at call time**. Both of those packages
...
import `datasheet_analyzer.families` first — the same reason `provenance.py`
imports `CorpusIndex` at call time.
"""
...
def load_members(part_numbers, parts_dir: Path) -> list[FamilyMember]:
    ...
    built around.
    """
    from datasheet_analyzer.compare.build import CompareRecord
    ...
    for part_number in part_numbers:
        ...
        out.append(
            FamilyMember(
                part_number=part_number,
                specs=tuple(
                    CompareRecord(doc=doc.name, record=record)
                ...
                pins=tuple(
                    MemberRecord(doc=doc.name, record=record)
                    for doc in index.docs
