...
def render_revision_diff(diff: RevisionDiff) -> str:
    ...
    lines.append("")
    lines.append(
        f"- **before:** {_side(diff.before_label, diff.before_revision)} — "
        f"`docs/{diff.before_doc}`"
    )
    lines.append(
        f"- **after:** {_side(diff.after_label, diff.after_revision)} — "
        f"`docs/{diff.after_doc}`"
    )
    lines.append("")
...
def _table(changes: list[RevisionChange]) -> list[str]:
    ...
    header = (
        "| What | Change | Before | After | Δ after − before | Aligned on | Pages |"
    )
    rows = [header, "|---|---|---|---|---|---|---|"]
