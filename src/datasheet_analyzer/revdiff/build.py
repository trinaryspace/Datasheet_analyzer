...
from datasheet_analyzer.config import CARD_VERSION, REVDIFF_SCHEMA_VERSION
...
class RevisionSide:
    ...
    `doc` is the document directory those records live in, which is what a fully
    qualified `source` reference names; `label` is what the caller selected the
    side by and `revision` what the document itself printed — the two are kept
    apart because a human's filing label is not a reading of the page.
    """
...
def _section_changes(before: RevisionSide, after: RevisionSide) -> list[RevisionChange]:
    ...
    for key, section in new.items():
        ...
        if previous.title.strip() != section.title.strip():
            changes.append(
                _change(
                    KIND_SECTION, CHANGE_RETITLED, key,
                    label=_section_label(section),
                    aligned_on=_section_aligned(section),
                    field="title",
                    ...
                    after=_section_value(after, section, "title"),
                    summary=(
                        f"§{key} retitled: {previous.title!r} -> {section.title!r}"
                    ),
                )
            )
        if (previous.page_start, previous.page_end) != (section.page_start, section.page_end):
            changes.append(
                _change(
                    KIND_SECTION, CHANGE_PAGE_SHIFTED, key,
                    label=_section_label(section),
                    aligned_on=_section_aligned(section),
                    field="page",
                    ...
                    summary=(
                        f"§{key} {section.title} moved: {_pages(previous)} -> "
                        f"{_pages(section)}"
                    ),
                )
            )
    for key, section in old.items():
        ...
        changes.append(
            _change(
                KIND_SECTION, CHANGE_REMOVED, key,
                label=_section_label(section),
                aligned_on=_section_aligned(section),
...
def _spec_pair_changes(
    ...
    *,
    ...
    for role in ROLE_ORDER:
        ...
        changes.append(
            _change(
                KIND_SPEC, CHANGE_CHANGED, display,
                label=_spec_label(record),
                aligned_on=aligned,
                field=role,
                before=old_value,
...
def _scalar(quantity) -> float | None:
    ...
    printed; a **range** states no single number and is never reduced to one of
    its endpoints to manufacture a delta — it is quoted verbatim and reviewed by
    hand. (`compare.build._scalar`, same rule, same reason.)
    """
...
def _ambiguous_specs(
    ...
    lines.extend(
        f"uncomparable: {key} / {_spec_label(row)} — {_spec_printed(row)} "
        f"({_page(row.page)})"
        for row in rows
    )
    return lines
...
def _pin_changes(before: RevisionSide, after: RevisionSide) -> list[RevisionChange]:
    ...
    for key, pin in new.items():
        ...
        if previous is None:
            changes.append(
                _change(
                    KIND_PIN, CHANGE_ADDED, pin.pin or key,
                    label=pin.name,
        ...
        if previous.name.strip() != pin.name.strip():
            changes.append(
                _change(
                    KIND_PIN, CHANGE_RENAMED, pin.pin or key,
                    label=pin.name,
                    aligned_on=ALIGNED_PIN,
                    field="name",
                    ...
                    after=_pin_value(after, pin, "name"),
                    summary=(
                        f"pin {pin.pin} renamed: {previous.name!r} -> {pin.name!r}"
                    ),
                )
            )
        for what in ("type", "direction", "description"):
            ...
            changes.append(
                _change(
                    KIND_PIN, CHANGE_CHANGED, pin.pin or key,
                    label=pin.name,
                    aligned_on=ALIGNED_PIN,
                    field=what,
                    ...
                    summary=(
                        f"pin {pin.pin} {what}: {old_cell or '(none)'} -> "
                        f"{new_cell or '(none)'}"
                    ),
                )
            )
    for key, pin in old.items():
        ...
        changes.append(
            _change(
                KIND_PIN, CHANGE_REMOVED, pin.pin or key,
                label=pin.name,
                aligned_on=ALIGNED_PIN,
...
def _pin_value(side: RevisionSide, pin: PinRecord, what: str) -> DerivedValue:
    ...
    A pin's type is the one field on the record that is not verbatim — it is a
    label from `registry/pin_types.yaml` — so its envelope records that rule
    rather than `copy_cell`, and the phrase that decided it travels in `section`
    nowhere: it stays on the record, which the source reference resolves to.
    """
    return DerivedValue(
...
def _register_changes(
    ...
    for key in list(new) + [k for k in old if k not in new]:
        ...
        for record in only_new:
            changes.append(
                _change(
                    KIND_REGISTER, CHANGE_ADDED, _register_display(record),
                    label=record.name,
        ...
        for record in only_old:
            changes.append(
                _change(
                    KIND_REGISTER, CHANGE_REMOVED, _register_display(record),
                    label=record.name,
                    aligned_on=ALIGNED_REGISTER,
...
def _register_pair_changes(
    ...
    if old_reset != new_reset:
        changes.append(
            _change(
                KIND_REGISTER, CHANGE_RESET, display,
                label=record.name,
                aligned_on=ALIGNED_REGISTER,
                field="reset",
                ...
                summary=(
                    f"register {display} {record.name} reset: {old_reset} -> "
                    f"{new_reset}"
                ),
            )
        )
    if previous.name.strip() != record.name.strip():
        changes.append(
            _change(
                KIND_REGISTER, CHANGE_RENAMED, display,
                label=record.name,
                aligned_on=ALIGNED_REGISTER,
                field="name",
                ...
                after=_register_value(after, record, "name"),
                summary=(
                    f"register {display} renamed: {previous.name!r} -> {record.name!r}"
                ),
            )
        )
    for what in ("access", "description"):
        ...
        changes.append(
            _change(
                KIND_REGISTER, CHANGE_CHANGED, display,
                label=record.name,
                aligned_on=ALIGNED_REGISTER,
                field=what,
                ...
                summary=(
                    f"register {display} {what}: {old_cell or '(none)'} -> "
                    f"{new_cell or '(none)'}"
                ),
            )
        )
...
def _field_changes(
    ...
    for key, field_record in new.items():
        ...
        if earlier.bits.verbatim.strip() != field_record.bits.verbatim.strip():
            changes.append(
                _change(
                    KIND_FIELD, CHANGE_CHANGED, label,
                    label=field_record.name,
                    aligned_on=ALIGNED_FIELD,
                    field="bits",
    ...
    for key, field_record in old.items():
        ...
        changes.append(
            _change(
                KIND_FIELD, CHANGE_REMOVED, label,
                label=field_record.name,
                aligned_on=ALIGNED_FIELD,
...
def _register_identity(record: RegisterRecord) -> frozenset[str]:
    return frozenset(
        text
        for text in (normalize(record.name), normalize(record.address.verbatim))
        if text
    )
...
def _register_value(
    side: RevisionSide, record: RegisterRecord, what: str
) -> DerivedValue:
...
def _field_value(
    ...
    return DerivedValue(
        ...
        section=record.section,
        derivation=DERIVATION_CELL,
        confidence=record.fields_confidence,
    )
...
def _delta(
    ...
    if before.value_si is None or after.value_si is None:
        unreadable = [
    ...
    if before.unit_si != after.unit_si:
        return None, (
    ...
    return (
        DerivedValue(
            value_si=after.value_si - before.value_si,
            unit_si=after.unit_si,
            source=after.source,
            sources=[before.source],
            page=after.page,
            section=after.section,
            derivation=f"{DERIVATION_DELTA}:{role}",
            confidence=weakest(after.confidence, before.confidence),
        ),
        "",
    )
...
def _ref(side: RevisionSide, record_id: str, artifact: str) -> str:
    """A record's fully qualified `source`, or `""` when it has no id.

    Part-qualified and document-qualified, for the reason a comparison's are
    (phase 6, ticket 09) and one more: **both** revisions of this part publish a
    `rec_1`, so a reference that named neither the document nor the part would
    resolve to a confident, wrong record — the earlier revision's row read as the
    later one's. A record with no id predates ADR 0005 and gets no reference
    rather than a fabricated one; the diff still reports the change.
    """
    if not record_id:
        return ""
    return source_ref(
        record_id, artifact=artifact, doc=side.doc, part=side.part_number
    )
...
def _notes(
    diff: RevisionDiff, before: RevisionSide, after: RevisionSide
) -> list[str]:
    ...
    for side in (before, after):
        population = parse_population(side.specs)
        valueless = sum(1 for record in side.specs if not _has_printed_value(record))
        lines.append(
            f"{side.label or side.doc}: {population.describe()} "
