...
```bash
dsa ask --part AFE7950 "max junction temperature" --budget 3000   # start here
dsa query --part AFE7950 --symbol TJ --json          # exact rows, rung + grade
dsa search --part AFE7950 "sysref setup" --limit 3   # cited sections
dsa plots --part AFE7950 --q "output power" --json   # figure catalog
```

Worked example — a question in a designer's words, a cited answer out:

```bash
$ dsa ask --part AFE7950 "what is the maximum junction temperature?"
```
...
```text
ask       {"part": "AFE7950", "question": "max junction temperature", "budget": 3000}
  -> {"pack": {...}, "citations": ["§<n>, p.<page>"], "tokens": 180, "truncated": false}
find_plots {"part": "AFE7950", "q": "output power"}
