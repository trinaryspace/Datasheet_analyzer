...
- **every other part has `url: null` plus the reason**, because no URL pattern
  for that vendor (or for TI's *user guide* path) is encoded anywhere in this
  repo and guessing one would put a wrong document in front of a designer with
  a citation that looks exactly as trustworthy as a right one. `dsa fetch`
  then asks for `--url`, which is the designed growth path.
...
#: The documents this repo actually holds, as (part, repo-relative path,
#: doc type). The repo-root copies are the documented `dsa build` working
#: files; LMX1204's two documents live only under `tests/fixtures/pdf/`.
SEED_DOCUMENTS: list[tuple[str, str, DocType]] = [
    ...
    ("LMX1204", "tests/fixtures/pdf/lmx1204.pdf", DocType.DATASHEET),
    ("LMX1204", "tests/fixtures/pdf/LMX1204_registermap.pdf", DocType.REGISTER_MAP),
]
...
def literature_number(path: Path) -> str:
    ...
    try:
        text = "\n".join(
            doc[i].get_text() for i in range(min(_LIT_PAGES, doc.page_count))
        )
    finally:
        doc.close()
